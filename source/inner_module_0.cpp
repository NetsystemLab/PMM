...
int get_node(int x, int &num, Line *node) {
  ...
  for (int i = 0; i < mo[x].rule; i++) {
    ...
    uint32_t tnx = 1<<nx;
#if INNER_MODULE_0_DEBUG
    printf("For the %dth tules, %s ",i,mo[x].c[i]);
    printf("tx = %d nx = %d\n", tx, nx);
#endif
    for (int j = 0; j < tnx; j++) {
      int ans = 0, idx = 0;
      ...
      if (last == len) {
        node[num].l = ans; node[num].r = ans; node[num].pri = mo[x].pri[i]; 
#if INNER_MODULE_0_DEBUG
        std::cout << num << " { " << node[num].l << " " << node[num].r << " " << node[num].pri << " }" << std::endl;
#endif
        num++;
      }
      else {
        node[num].l = ans<<tx; node[num].r = (ans<<tx)+((1<<tx)-1);  node[num].pri = mo[x].pri[i];
#if INNER_MODULE_0_DEBUG 
        std::cout << num << " { " << node[num].l << " " << node[num].r << " " << node[num].pri << " }" << std::endl;
#endif
        num++;
...
#if INNER_MODULE_0_DEBUG
          std::cout <<"nx["<<numx-1<<"].r="<<nx[numx-1].r<<"  "<<"n1["<<i<<"].l="<<n1[i].l<< std::endl;
#endif
